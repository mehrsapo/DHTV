...
class CallbackShowConvergence(Callback):
    ...
    def __init__(self, L, H, values, lm, step, file_name, title='convergence', logx=False, logy=False,
                 **kwargs):
        """Initialize a new instance.
    ...
    def __call__(self, x):
        ...
        if self.iter % self.step == 0:
            ...
            y_lmbda = self.H.dot(x)
            df_loss = 1/ 2 * ((y_lmbda - self.values) ** 2).sum()
            total_loss = df_loss + self.lambdaa_admm*htv_loss
            print(df_loss, htv_loss, total_loss)
            with open(self.file_name, 'a') as file:
                file.write(str(df_loss) + ' ' + str(htv_loss) + ' ' + str(total_loss) + '\n' )

            # self.ax.scatter(it, htv_loss, **self.kwargs)
        self.iter += 1
