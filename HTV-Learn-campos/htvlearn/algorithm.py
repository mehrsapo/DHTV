...
class Algorithm():
    ...
    def run_admm(self):
        ...
        t1 = time.time()
        print(f'Campos face htv learn time {t1-t0} seconds')

        points = self.data.train['input']

        np.savetxt('saved_for_comparison/face_train_points_campos.txt', points)
        np.savetxt('saved_for_comparison/face_train_values_campos.txt', self.data.train['values'])

        np.savetxt('saved_for_comparison/face_valid_points_campos.txt', self.data.valid['input'])
        np.savetxt('saved_for_comparison/face_valid_values_campos.txt', self.data.valid['values'])

        np.savetxt('saved_for_comparison/face_test_points_campos.txt', self.data.test['input'])
        np.savetxt('saved_for_comparison/face_test_values_campos.txt', self.data.test['values'])

        grid_points = self.lat.lattice_to_standard(torch.unique(
                    self.lat.unique_simplices.view(-1, 2).float(), dim=0))

        np.savetxt('saved_for_comparison/face_grid_points_campos.txt', grid_points)

        grid_values = self.lat.flattened_C

        np.savetxt('saved_for_comparison/face_grid_values'+str(self.lmbda)+'.txt', grid_values)
    ...
    def setup_admm(self):
        ...
        sparse.save_npz('saved_for_comparison/L_face_campos.npz', self.op.L_mat_sparse / self.lmbda)
        sparse.save_npz('saved_for_comparison/H_face_campos.npz', self.op.H_mat_sparse)
        ...
        data_fit = \
            1/2 * odl.solvers.L2NormSquared(H_op.range).translated(x_values_np)
        ...
        callback = (odl.solvers.CallbackPrintIteration(step=2000) &
            odl.solvers.CallbackShowConvergence(self.op.L_mat_sparse / self.lmbda,  self.op.H_mat_sparse, x_values_np, self.lmbda, 
            step=2000, file_name='saved_for_comparison/face_loss'+str(self.lmbda)+'.txt'))
